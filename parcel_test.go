...
func TestAddGetDelete(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, parcel.Client, client.Client)
	require.Equal(t, parcel.Status, client.Status)
	require.Equal(t, parcel.Address, client.Address)
	require.Equal(t, parcel.CreatedAt, client.CreatedAt)
	...
	err = store.Delete(id)
	require.NoError(t, err)
	clientNil, err := store.Get(id)
	require.Error(t, err)
	require.Empty(t, clientNil.Client) //Не лучший способ для проверки, так как store.Get(id) не может вернуть nil
...
func TestSetAddress(t *testing.T) {
	...
	client, err := store.Get(id)
	require.NoError(t, err)
	require.Equal(t, newAddress, client.Address)
}
...
func TestSetStatus(t *testing.T) {
	...
	client, err := store.Get(id)
	require.NoError(t, err)
	require.Equal(t, ParcelStatusDelivered, client.Status)
}
